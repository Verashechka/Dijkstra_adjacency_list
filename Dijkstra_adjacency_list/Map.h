...
private:
	...
	void clear_map(node<T, L>* current); // очищение ассоциативного массива
	list<T>* get_key(list<T>*, node<T, L>* current);
	list<L>* get_data(list<L>*, node<T, L>* current);
	void delete_node(node<T, L>* active_node);
	void balance(node<T, L>* active_node);
	node<T, L>* swap(node<T, L>* active_node);
	void black_uncle(node<T, L>* active_node);
	node<T, L>* left_rotate(node<T, L>* active_node);
	node<T, L>* right_rotate(node<T, L>* active_node);
	void double_black(node<T, L>*, bool is_fiction);
	void red_brother_case(node<T, L>* db_node, bool is_fiction);
	void black_nephews_case(node<T, L>* db_node, bool is_fiction);
	void red_nephew_case(node<T, L>* db_node, bool is_fiction);
...
public:
	...
	node<T, L>* find(T key); // поиск элемента по ключу
};
...
template<class T, class L> //done
node<T, L>* map_rb_tree<T, L>::right_rotate(node<T, L>* active_node)
{	// rotates root of subtree right
...
template<class T, class L>
void map_rb_tree<T, L>::output_adjacency_list()
{
	HANDLE hStdOut = GetStdHandle(STD_OUTPUT_HANDLE);
	system("cls");
	vector<node<T, L>*> Queue;
